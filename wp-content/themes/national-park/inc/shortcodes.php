 ...
 */
function npx_ads_script_layout( $atts ) {
	ob_start();
	...
	if ( 'horizontal-banner' === strtolower( $atts['layout'] ) ) {
		if ( get_field( 'horizontal_banner', 'options' ) ) {
			...
			?>
				<div class="add-banner">
					<img src="<?php echo esc_url( NPX_THEME_URI . '/dist/assets/images/addbanner.png' ); ?>" alt="add-banner" />
				</div>
			<?php
		}
	}
		...
		if ( get_field( 'vertical_banner', 'options' ) ) {
			...
			?>
				<div class="add-banner <?php echo esc_attr( $atts['class'] ); ?>">
					<img class="img-cover" src="<?php echo esc_url( NPX_THEME_URI . '/dist/assets/images/sidebar-ads.png' ); ?>" alt="detail-img" />
					<img class="img-cover" src="<?php echo esc_url( NPX_THEME_URI . '/dist/assets/images/sidebar-ads.png' ); ?>" alt="detail-img" />
				</div>
			<?php
		}
...
add_shortcode( 'ads_section_layout', 'npx_ads_script_layout' );
